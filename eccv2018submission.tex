...
\section{Introduction}
    ...
    \begin{itemize}
        ...
        \item multimodal \textbf{baseline features} are extracted from the model to be fed a classifier for error prediction;
...
The model facet set is noted $\mathsf{F}$. $\forall (f, g) \in \mathsf{F} \times \mathsf{F} \quad f \sim g$ correspond to facets $f$ and $g$ being adjacent: \textit{i.e.} they share a common edge. As the roof topology graph in~\cite{Verma2006}, the input building model can be seen as a facet (dual) graph:
\begin{equation}
...
We applied this new framework to the case of aerial urban reconstruction. Orthorectified images and a \acrshort{acr::dsm} are used in this special for feature extraction. A dataset containing $1501$ diversified aerially reconstructed building models was experimented on using the devised evaluation method and a multimodal baseline features. Although, results are mitigated, they are persuasive enough to continue investigating the present evaluation method. The next step would be to propose more structurally aware features to apply on a richer more diverse dataset.
