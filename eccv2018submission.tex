...
\begin{abstract}
    The automatic generation of 3D urban models from geospatial data is now a standard procedure in industry. However, practitioners still have to visual assess, at the scale of cities, the correctness of these models so as to detect the inevitable reconstruction errors. Such process relies on experts, and is highly time-consuming (typically, 2h/km$^2$ per expert). In this work, we propose a novel approach for automatically evluating the quality of 3D models of buildings. Errors and defects present in 3D models are compiled in a hierarchical, versatile, and parameterizable taxonomy. This allows for the first time to disentangle fidelity and modelling errors, whatever the level of details of the reconstructed buildings. The quality of 3D models is predicted using the intrinsic geometric properties of building and, when available, on image and depth data. A baseline of handcrafted yet generic  features is fed into a supervised Random Forest classifier. Both multi-class and multi-label cases are predicted. Due to the high interdependence between errors, we have the ability to retrieve all errors. . and tested over dataset containing more that one thousand models. It can detect on average $96\%$ of well-balanced errors. These results motivates further work devising better feature extraction methods over a more well balanced and rich dataset.
...
\begin{table}
	...
	\begin{center}
        \begin{tabular}{|x{1.4cm} | x{1.2cm} x{1.2cm} | x{1.2cm} x{1.2cm} | x{1.2cm} x{1.2cm} | x{1.2cm} x{1.2cm}|}
			\hline
            &\multicolumn{2}{x{2.4cm}|}{\textbf{Geometry}} & \multicolumn{2}{x{2.4cm}|}{\textbf{Geometry $\cup$ Height}} & \multicolumn{2}{x{2.4cm}|}{\textbf{Geometry $\cup$ Image}} & \multicolumn{2}{|x{2.4cm}|}{\textbf{All}}\\
            \cline{2-9}
