...
\begin{abstract}
	We are tackling the problem of 3D model reconstruction quality evaluation. Currently, there is no scalable way of assessing the output of automatic recontruction methods, as one must rely on the presence of high precision models that are not easily affordable. In our approach, we try to predict the models quality based on its geometry and available remote sensing data.
	\dots
